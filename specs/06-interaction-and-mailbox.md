...
- **High-stakes work** — work that is **constitutional** (`00` §6), **irreversible** (by the predicate above, *including the unknown-reversibility case*), or carries **significant budget** is **never silently auto-defaulted**. It follows the uniform **bounded escalation-timeout** policy: the Question is held for a bounded escalation window and, on expiry, re-routed up the cross-user precedence order (§3.7) to the next authorized principal. If the window elapses with no authoritative Answer, the gated node **holds and degrades safely** — it stays `Blocked`, sheds its dependent work cleanly, and emits a `Warning` Notification — but **never proceeds on the irreversible action**. This safe-held state is one of the node's **active gating edges** (§3.4): if the same node is also gated by other Questions, resolving any *one* of them does **not** release it — the node stays `Blocked` until *every* active edge, this fallback included, is cleared. There is no auto-default that *acts*: the only fallback is to stay safely stalled, honoring the bootstrap ("do not proceed until answered") without dead-ending availability for the rest of the system. No high-stakes path blocks *indefinitely* — every wait is bounded and resolves to this defined safe fallback.
...
- **a node's `Blocked` state is the conjunction over *all* of its currently-active gating edges, not the state of any single Question.** When more than one Question gates the same node, the node stays `Blocked` while *any* of them is still active; answering or closing one Question clears **only that Question's own edge**, and the node returns to `Ready` only once **no** active gating edge remains. A gating edge is **active** while its Question is `OPEN` **and also** while its Question has safe-held into the bounded high-stakes fallback of §2.4 — a Question that has `CLOSED` by escalation-timeout still holds its node `Blocked` ("never proceeds on the irreversible action"), so its edge stays active until it is authoritatively answered or superseded. This matters precisely when several Questions gate one node and resolve into *different* states at *different* times (e.g. one still-`OPEN`, one safe-held-`CLOSED`): resolving the `OPEN` one must not release a node the safe-held one still holds. (The common single-Question-per-node case is unaffected — that node clears exactly when its one Question resolves.)
