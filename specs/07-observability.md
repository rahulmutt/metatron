...
Whether even the never-sampled spine floor is affordable at target scale, and whether making the interior rate a PID-controlled dimension is dynamically stable, remain **open (§6)**.
...
2. **Stability of a PID-controlled interior sampling rate.** §5.4 makes the interior sampling rate a PID-influenced knob with a hard floor. Whether promoting observability cost to a *controlled dimension* of the PID is **dynamically stable** — rather than inducing oscillation or starving the very signal the controller reads — is a control-design question left open.
...
- **06-interaction-and-mailbox** — Roots every causal chain at `InstructionReceived`/`GoalNormalized`; the mailbox-block duration (`interaction.question_blocked_seconds`) feeds the PID **latency** dimension; instruction-content privacy is resolved as **hash-in-trace / body-out-of-band** (§5.1), with the body store shared with 06.
- **08-trust-and-security** — Spine events are **signed** (audit-grade); Sentinel alerts feed **reputation** decay; the access-controlled instruction-body store (§5.1) and the bounded Sentinel **emergency authority** (§5.5) live in 08's trust model. Sentinels are the watch role from taxonomy §3, but remain inside the constitutional loop (no direct *structural* state mutation; their emergency actions are enumerated and auto-ratified, §5.5).
