...
1. **Machine consumers, in-loop (first-class).** Two named internal consumers close the control loop:
   ...
   - **steering-loop estimators (03)** consume metrics to compute the **`ErrorVector`**, including consensus **`dispersion`** as the **divergence** signal and **verification coverage** — the fraction of a decision that was machine-verifiable — as a first-class companion metric (§3.4).
...
| **Metric** | A **time series** — a named, dimensioned numeric sample stream. | Aggregatable. | High-rate, downsampled. | steering-loop estimators (03). |
...
| `EpisodeId` | one cause→effect→correction arc | first proposal of an arc | groups successive `TraceId`s the steering loop chained together |
...
Sensitive instruction **bodies never ride the telemetry itself**: traces carry only the `InstructionId` and a content **hash**, with the body held in a separate access-controlled store and joined by id — exactly as the chain joins the Merkle node by `CommitHash` (§5.1).
...
└───────────────┬───────────────┬───────────────┬───────────────────────────┘
   ...
   └───┬─────────────────────┬───────────────────┬───────────┘
       ▼                     ▼                   ▼
  ┌─────────┐         ┌──────────────┐   ┌──────────────────────┐
  │SENTINELS│         │ STEERING LOOP │   │ HUMANS / EXTERNAL OTel│
...
- **Metrics:** `consensus.proposals_total{outcome}`; `consensus.votes_total{verdict}`; `consensus.posterior` (histogram); **`consensus.dispersion` (the divergence source)**; **`consensus.verification_coverage` (gauge — fraction of the decision that was machine-verifiable, §3.4)**; `consensus.confident_error_risk` (gauge — the composite low-dispersion/low-coverage signal, §3.4); `consensus.rounds` (histogram — deliberation cost); `consensus.verify_failures_total`; `control.error_vector{dim}` (gauge, one series per dimension); `control.action_magnitude{dim}`.
...
Telemetry bus ─▶ stream subscription (filtered by event kind & metric name)
   ...
   └─ trap-rate monitor ◀─ exec.trap_rate{agent}, exec.deopt_total
        ...
        SentinelAlertRaised ──┼──▶ Reputation update (08): drifting agents decay
                              │      — gated on k-of-n Sentinel corroboration
                              │        or G0 adjudication (§5.9); never one alert
                              └──▶ Tier demotion (05): rising trap rate ⇒ deopt /
                                   demote a Tier-1/2 agent back toward Tier-0
```

Sentinels **do not mutate state directly** — separation of powers (§3 taxonomy) holds. They raise alerts and feed reputation/tier signals; any *structural* response (quarantine, rewire, demote) is a **typed proposal** decided by consensus (02), or an automatic deopt governed by 05's trap contract. Beyond that, Sentinels hold a narrow **enumerated, rate-limited, recorded, auto-ratified emergency authority** (e.g. emergency deopt on a trap storm) — bounded as in §5.5. This keeps even the watchdog inside the constitutional loop.

**No single Sentinel may move reputation or vote weight (ROB-06).** Reputation feeds voting weight (08), so an unaccountable path from one watcher into reputation would let a single drifting or compromised Sentinel shift the weighted majority **without holding a Genesis key** — defeating the *propose ≠ dispose* / *no single agent swings consensus* invariant. Therefore a **single Sentinel finding never, by itself, moves reputation or vote weight**: a reputation-affecting finding requires **k-of-n Sentinel corroboration** (independent Sentinels concurring) **or** adjudication through the **deterministic verification gate (G0)** before it touches 08's reputation. The auto-ratified emergency authority above is correspondingly bounded to **reversible actions only** (e.g. emergency deopt) — **never** a reputation or vote-weight change that would bias future consensus (§5.5, §5.9). This preserves the *no single agent swings consensus* invariant on the Sentinel path.

### 3.4 Data path to the steering-loop estimators (03)

The steering-loop estimators subscribe to the **Metric** view and compute the `ErrorVector` (00 §7). Mapping is explicit:
...
`divergence` deserves emphasis: it is **not** re-derived by the observability plane. The consensus protocol computes `Decision.dispersion` (00 §7: *"how split the council was -> feeds steering-loop divergence"*). The observability plane merely **transports** it as the `consensus.dispersion` metric and the steering loop reads it. This keeps the divergence signal authoritative and single-sourced.
...
Metatron's *internal* consumers (Sentinels, steering loop) read from the native bus for low latency; *external* consumers attach via the Collector. The two never diverge because both are fed the same OTLP stream.

### 3.7 Storage & retention

Three logical stores, each tuned to its kind (the event log and the Merkle DAG share one content-addressed substrate under separate retention namespaces — §5.2):
...
The **relationship between the event log and the Merkle commit history** is **resolved** (§5.2): they **share one content-addressed store** — a single immutable truth joined by `CommitHash` — under **separate retention namespaces**; §2.3's rule that they are *joined*, not duplicated, still holds.
...
- **High-cardinality interior telemetry is sampled.** Harness `token_stream` and `tool_call` spans use **tail-based sampling** keyed on outcome: keep 100% of traces that hit an error, a trap, a deopt, or high dispersion; sample the boring successful path. This keeps cost bounded while preserving every *interesting* trace. The interior sampling rate is a **steering-loop-influenced knob with a hard floor** (observability cost as a *minor* control input that can never starve the signal to zero) — §5.4.
...
Whether even the never-sampled spine floor is affordable at target scale, and whether making the interior rate a steering-loop-controlled dimension is dynamically stable, remain **open (§6)**.
...
This is a case where the observability plane must actively *protect* a governance invariant rather than passively report. Recorded immutably (§6.6), but **revealed on a schedule that preserves decorrelation (§6.3).** Operators who must watch consensus *as it happens* use a privileged **human-only live view air-gapped from agent-readable telemetry** (§5.6).
...
trait ObservabilityQuery {
    ...
    fn metrics(&self, q: MetricQuery) -> TimeSeries;              // steering-loop estimators
...
/// steering-loop estimator input contract (§3.4) → computes ErrorVector (03).
trait ControlFeed {
...
A design review **resolved** the questions previously parked in this spec. The decisions below are **normative committed design**, not options. The two genuinely-open items that remain are in §6.

1. **Instruction-content privacy — hash-in-trace, body out-of-band.** Telemetry and traces carry only an `InstructionId` plus a content **hash** of the instruction; the instruction **body lives in a separate access-controlled store** referenced by that id. Sensitive user text is therefore **never spilled** into broadly-readable telemetry or external OTel exports (§3.6), while **full causal-chain reconstruction** (§2.4) and the immutable audit trail are preserved — the chain joins to the body by id exactly as it joins to the Merkle node by `CommitHash`. Cross-ref **08** (access control on the body store) and **06** (instruction intake).
2. **Storage — one content-addressed store, separate retention namespaces.** The immutable **event log** and the **Merkle DAG (01)** **share a single content-addressed store** — one immutable source of truth, joined by `CommitHash` — under **separate retention namespaces** so each keeps its own eviction/tiering policy (§3.7). §2.3's rule that telemetry *joins* the Merkle history and never *duplicates* it still holds.
3. **Clock & ordering — Lamport for causality, HLC for display.** Causal/partial order is established by **01's Lamport logical clock**, the authoritative primary key (§2.4). For human-readable **total-order** timelines, a **hybrid logical clock (HLC)** breaks ties deterministically. The HLC is **display-only and non-authoritative**: it never overrides logical-time causality and is consistent with 01's logical-time model.
4. **Sampling — spine never sampled; interior tail-sampled under a steering-loop-influenced floor.** **Spine events are never sampled** (§3.8). **Interior** telemetry (harness `token_stream`/`tool_call`) is **tail-sampled**, and the interior sampling rate is a **steering-loop-influenced knob with a hard floor** — observability cost is admitted as a *minor* control input, but the floor guarantees the rate can never be driven to zero. (Affordability of the spine floor and stability of the steering-loop-controlled interior dimension remain open — §6.)
5. **Sentinel authority — enumerated, auto-ratified emergency actions.** Sentinels may trigger an **enumerated, rate-limited, recorded, auto-ratified** set of **emergency actions** (e.g. emergency deopt on a trap storm) without a prior consensus round; the set is closed and every invocation is itself a signed spine event. **Structural** change (quarantine, rewire, demote) still goes through **consensus (02)**. This is consistent with the blast-radius-scaled JIT autonomy in **05**: emergency power is bounded to immediate, reversible, locally-scoped safety responses — **strictly reversible actions only (e.g. deopt); never a reputation or vote-weight change** that would bias future consensus (see §5.9).
6. **Embargo vs. dashboard — a human-only, air-gapped live view.** The blind-vote embargo (§3.9) is preserved for all **agent** consumers. Operators get a privileged **human-only live consensus view** that is **air-gapped from agent-readable telemetry** — humans may watch consensus as it happens because they do **not** feed back into agent decisions, so observing in-flight votes cannot correlate the council. The **residual risk** — a human relaying what they see back into the loop out-of-band — is documented and accepted as a human-process concern, not a plane-enforced guarantee.
7. **Coverage semantics — defined against declared capability.** `coverage_ratio` (§3.5) is defined against a source's **declared** capability. A harness that emits **more** than declared **updates its declared capability** (the drift is recorded as an event); **under-declaring is fine** (extra telemetry is a bonus, not a violation); and a genuine shortfall is the explicit `Coverage::Gap` (§4.1). "Observed less than declared" and "declared less than observed" are thus both first-class, queryable, and never silent.
8. **Verification coverage is a first-class control metric (ROB-01).** Alongside `dispersion`, the plane exposes **`consensus.verification_coverage`** — the fraction of each decision that was machine-verifiable under the deterministic gate — and a composite **`consensus.confident_error_risk`** signal combining **low dispersion with low coverage**. The control loop (03) reads this composite as an **escalating risk signal, not a healthy reading**, so the worst failure mode — a **correlated, confidently-wrong council**, which emits *low* dispersion — can no longer masquerade as the healthiest telemetry (§3.4). Cross-ref **03** (escalation response) and **02** (decorrelated red-team lane on high-blast-radius proposals).
9. **No single Sentinel moves reputation or vote weight (ROB-06).** Because reputation **is** voting weight (08), a single Sentinel finding **never** moves reputation or vote weight by itself: a reputation-affecting finding requires **k-of-n Sentinel corroboration** or adjudication through the **deterministic verification gate (G0)** before it touches reputation (§3.3). Auto-ratified Sentinel **emergency authority is bounded to reversible actions only** (e.g. deopt) and may **never** effect a reputation/weight change that biases future consensus (§5.5). This preserves the *no single agent swings consensus* invariant on the Sentinel side channel.

---

## 6. Open questions & ambiguities

These remain **genuinely open** after the design review; everything else moved to §5 (00 §9 convention).

1. **Target-scale telemetry budget.** §5.4 fixes that spine events are never sampled, but the affordability of even that **never-sampled spine floor** at very large org-charts is **unproven** — an empirical question of target-scale budget and the crossover where the spine itself becomes the cost bottleneck.
2. **Stability of a steering-loop-controlled interior sampling rate.** §5.4 makes the interior sampling rate a steering-loop-influenced knob with a hard floor. Whether promoting observability cost to a *controlled dimension* of the steering loop is **dynamically stable** — rather than inducing oscillation or starving the very signal the controller reads — is a control-design question left open.
...
- **01-state-model** — Telemetry **joins** the Merkle DAG via `CommitHash` rather than duplicating it; uses 01's **logical time** as the primary ordering key. Storage is now **shared** with the Merkle DAG under separate retention namespaces (§5.2), and ordering uses 01's Lamport clock with a display-only HLC tie-break (§5.3) — both consistent with 01.
- **02-consensus** — Consumes `Proposal`/`Vote`/`Decision`; transports `Decision.dispersion` as the authoritative `divergence` signal **and `verification_coverage` as its first-class companion** (the composite low-dispersion/low-coverage *confident-error* signal, §3.4, §5.8); **protects** the blind-vote decorrelation invariant via the embargo (§3.9). Sentinel alerts feed reputation (and thus vote weight) **only after k-of-n corroboration or G0 adjudication**, so no single Sentinel swings the weighted council (§5.9).
- **03-control-loop** — The steering-loop estimators are a **named first-class consumer** (§3.4). Observability supplies every `ErrorVector` component as a control metric and emits `ErrorVectorSampled` / `ControlActionEmitted`, closing the loop's telemetry seam (`derived_from`). The composite **confident-error** signal (low dispersion + low coverage) is delivered as an **escalating** risk input the controller must act on, never relax against (§3.4, §5.8).
...
- **06-interaction-and-mailbox** — Roots every causal chain at `InstructionReceived`/`GoalNormalized`; the mailbox-block duration (`interaction.question_blocked_seconds`) feeds the steering-loop **latency** dimension; instruction-content privacy is resolved as **hash-in-trace / body-out-of-band** (§5.1), with the body store shared with 06.
- **08-trust-and-security** — Spine events are **signed** (audit-grade); Sentinel alerts feed **reputation** decay **only after k-of-n Sentinel corroboration or G0 adjudication** — a single Sentinel finding never moves reputation or vote weight (§5.9); the access-controlled instruction-body store (§5.1) and the bounded Sentinel **emergency authority** (§5.5, reversible actions only) live in 08's trust model. Sentinels are the watch role from taxonomy §3, but remain inside the constitutional loop (no direct *structural* state mutation; their emergency actions are enumerated, auto-ratified, and **reversible-only**, §5.5).
